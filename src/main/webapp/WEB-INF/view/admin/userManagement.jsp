...
<link href="${path}/resources/css/admin/userManagement.css"
	rel="stylesheet">
	...
	<div class="container-fluid py-4">
		<div class="table-container shadow-sm p-4 bg-white rounded">
			...
			<div class="d-flex justify-content-between align-items-center mb-4">
				<div>
					<h5 class="mb-0">
						<strong>총 회원 수:</strong> <span class="text-primary">${totalCount}</span>명
					</h5>
				</div>
							...
							<c:when test="${not empty li}">
								<c:forEach var="m" items="${li}">
									<tr>
										...
										<td>${m.userName}</td>
										<td>${m.fullAddress}</td>
										<td><fmt:formatNumber value="${m.totalSpent}"
												pattern="#,###" />원</td>
										<td><a
											href="${path}/admin/userPurchaseDetail.do?userIdx=${m.userIdx}">
												<button type="button"
													class="btn btn-sm btn-outline-secondary">이력</button>
										</a></td>
									</tr>
								</c:forEach>
							</c:when>
