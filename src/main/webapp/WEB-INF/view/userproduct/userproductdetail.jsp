...
<div class="main-content-area">
  ...
  function updateRowTotal(row) {
      ...
      document.querySelectorAll(`input[name='quantityList'][data-option-idx='\${optionIdx}']`).forEach(el => el.value = qty);
      updateTotal();
  }
  ...
  function updateTotal() {
      ...
      document.querySelectorAll("#optionTable tbody tr").forEach(row => {
          total += (parseInt(row.querySelector("input[type='number']").value) * parseInt(row.dataset.price));
      });
  ...
  function openTab(tabId, btn) {
      ...
      window.scrollTo({ top: document.getElementById(tabId).offsetTop - 120, behavior: "smooth" });
  }
