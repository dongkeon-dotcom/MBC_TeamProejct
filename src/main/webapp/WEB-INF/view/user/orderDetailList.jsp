...
<section>
   ...
   <form method= "post" action="${path}/user/cart.do">
        ...
        <c:forEach var="m" items="${orderli}" varStatus="status">
          <tr> <td><a href="${path}/product/orderList.do?orderIdx=${m.orderIdx}">${m.orderIdx}</a> </td> <!-- 링크 달아서 상세 주문리스트로 이동해야함  -->
          <td>${m.totalPrice} </td>
          <td>${m.orderRegDate} </td><td><input type=button onClick="creview()" value="삭제 "></td></tr>
        </c:forEach>
        </c:when>
        <c:otherwise>
...
function addressDel(){
	...
	alert("주소삭")
	location.href="${path}/user/review.do?";
}
